        ...
        <div>
            <div className="flex items-center justify-center w-full">
                <label htmlFor="dropzone-file" className="flex flex-col items-center justify-center w-full h-64 bg-neutral-secondary-medium border border-dashed border-default-strong rounded-base cursor-pointer hover:bg-neutral-tertiary-medium">
                    ...
                    <input accept='image/png, imag/jpeg, image/gif' onChange={handleFileUpload} multiple id="dropzone-file" type="file" className="hidden" />
                </label>
            </div>
            <div className='grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 lg:grid-cols-7 xl:grid-cols-10 gap-3 mt-3'>
                {imagePreview.map((image, index) => (
                    <div key={index}>
                        <Image src={image} width={100} height={100} className="rounded-lg object-cover h-[100px] w-[100px]" alt={index} />
                    </div>
                ))}
            </div>
