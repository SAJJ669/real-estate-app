    ...
    return (
        <div>
            <div className='p-3 flex items-center w-full justify-center'>
                ...
                <Button variant="brand" className="flex gap-2" onClick={handleSearchClick}><Search />Search</Button>
            </div>
            <div>
                <FilterSection setBedroom={setBedroom} setBathroom={setBathroom} setParking={setParking} setHomeType={setHomeType} />
            </div>
            <div className='px-3 my-5'>
                {address && <h2 className='text-xl text-gray-400'>Showing {listing?.length} results for <span className='text-brand-purple font-bold'>{address?.address.split(',')[0]?.trim()}</span></h2>}
            </div>
